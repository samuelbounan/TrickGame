// 4-player card program using expert knowledge, expert games, belief states,
...
card GamePlay::lower_value(int index, card trumpmask) { return lower[index]; }

card GamePlay::higher_value(int index, card trumpmask) { return higher[index]; }

int GamePlay::has_largest_trump(card hand, card reduced, card played) {
  if (count(reduced & trumpmask) == 0) return 0;
  int l = largest_trump(reduced & trumpmask);
  if (count(trumpmask & ~hand & ~played) == 0) return 1;
  int o = largest_trump(trumpmask & ~hand & ~played);
  if (((ONE << o) & higher_value(l, trumpmask))) return 0;
  return 1;
}
...
int GamePlay::leading(int j, int issuer, card trumpmask) { return issuer; }
...
int GamePlay::win(int j, int k, int l, int m, card trumpmask) {
  ...
  if ((((ONE << j) | (ONE << k) | (ONE << l) | (ONE << m)) & trumpmask)) {
    //    cout << "winning card trmpcard " << card_name[j]<<
    //    card_name[k]<<card_name[l]<<card_name[m] << endl;
    ...
    //    cout << "inversed ordering " << inverse [w]<< ":" << inverse[j]<<" "
    //    << inverse[k]<< " "<<inverse[l]<< " "<<inverse[m] << endl; cout <<
    //    "total ordering " << total [w]<< ":" << total[j]<<" " << total[k]<< "
    //    "<<total[l]<< " "<<total[m] << endl;

    w = (((ONE << j) & trumpmask) && Card[j] != Card[w] &&
         inverse[j] < inverse[w])
            ? j
            : w;
    w = (((ONE << k) & trumpmask) && Card[k] != Card[w] &&
         inverse[k] < inverse[w])
            ? k
            : w;
    w = (((ONE << l) & trumpmask) && Card[l] != Card[w] &&
         inverse[l] < inverse[w])
            ? l
            : w;
    w = (((ONE << m) & trumpmask) && Card[m] != Card[w] &&
         inverse[m] < inverse[w])
            ? m
            : w;
    return w;
  ...
  int w = j;
  w = (Suit[w] == Suit[k] && Card[w] != Card[k] && inverse[k] < inverse[w]) ? k
                                                                            : w;
  w = (Suit[w] == Suit[l] && Card[w] != Card[l] && inverse[l] < inverse[w]) ? l
                                                                            : w;
  w = (Suit[w] == Suit[m] && Card[w] != Card[m] && inverse[m] < inverse[w]) ? m
                                                                            : w;
  return w;
}
...
int GamePlay::win(int j, int k, int l, card trumpmask) {
  ...
  if ((((ONE << j) | (ONE << k) | (ONE << l)) & trumpmask)) {
    ...
    w = (((ONE << j) & trumpmask) && Card[w] != Card[j] &&
         inverse[j] < inverse[w])
            ? j
            : w;
    w = (((ONE << k) & trumpmask) && Card[w] != Card[k] &&
         inverse[k] < inverse[w])
            ? k
            : w;
    w = (((ONE << l) & trumpmask) && Card[w] != Card[l] &&
         inverse[l] < inverse[w])
            ? l
            : w;
    return w;
  }
  //  cout << "no trump " << suitorder[j] << ","  << suitorder[k] << ","  <<
  //  suitorder[l]  << "," << endl;
  int w = j;
  w = (Suit[w] == Suit[k] && Card[w] != Card[k] && inverse[k] < inverse[w]) ? k
                                                                            : w;
...
int GamePlay::equivalent_card(card hand, int index, card playednottable) {
  ...
  //  if (!(hand & (ONE<<index))) { print_vector(hand,trumpmask); exit(1); } //
  //  good for debugging
  if (index > 0 && (hand & (ONE << index - 1)) && equiv[index][index - 1])
    return 0;
  if (index > 1 && (ex & (ONE << index - 1)) && (hand & (ONE << index - 2)) &&
      equiv[index][index - 2])
    return 0;
  //#ifndef BELOTE
  if (index > 2 && ex == (ex & ((ONE << index - 1) | (ONE << index - 2))) &&
      (hand & (ONE << index - 3)) && equiv[index][index - 3])
    return 0;
  if (index > 3 &&
      ex == (ex &
             ((ONE << index - 1) | (ONE << index - 2) | (ONE << index - 3))) &&
      (hand & (ONE << index - 4)) && equiv[index][index - 4])
    return 0;
  if (index > 4 &&
      ex == (ex & ((ONE << index - 1) | (ONE << index - 2) |
                   (ONE << index - 3) | (ONE << index - 4))) &&
      (hand & (ONE << index - 5)) && equiv[index][index - 5])
    return 0;
  if (index > 5 &&
      ex ==
          (ex & ((ONE << index - 1) | (ONE << index - 2) | (ONE << index - 3) |
                 (ONE << index - 4) | (ONE << index - 5))) &&
      (hand & (ONE << index - 6)) && equiv[index][index - 6])
    return 0;
  if (index > 6 &&
      ex == (ex &
             ((ONE << index - 1) | (ONE << index - 2) | (ONE << index - 3) |
              (ONE << index - 4) | (ONE << index - 5) | (ONE << index - 6))) &&
      (hand & (ONE << index - 7)) && equiv[index][index - 7])
    return 0;
  if (index > 7 &&
      ex ==
          (ex & ((ONE << index - 1) | (ONE << index - 2) | (ONE << index - 3) |
                 (ONE << index - 4) | (ONE << index - 5) | (ONE << index - 6) |
                 (ONE << index - 7))) &&
      (hand & (ONE << index - 8)) && equiv[index][index - 8])
    return 0;
  //#endif
  return 1;
}
...
card GamePlay::playable_cards(card hand, card played) {
  // cout << "playcards/0 ";
  // print_vector(hand, trumpmask);
  // cout << " ";
  // print_vector(played, trumpmask);
  // cout << endl;
  card playable = hand;
#ifdef HEART
  if (count(played) == 0)
  ...
  // cout << " before reducing/0 ";
  // ::print_extvector(playable, trumpmask);
  // cout << " " << endl;
  while (reduced) {
...
card GamePlay::playable_cards(card hand, int issuer, card played, card table,
                              int* i) {
  //  cout << "playcards/1 "; cout << issuer; cout << ":";
  //  ::print_vector(hand,trumpmask); cout << " played ";
  //  ::print_vector(played,trumpmask); cout << " table ";
  //  ::print_vector(table,trumpmask); cout << endl;
  ...
  if (shown == 1)
    ...
    int w = win(index, i[(issuer + 1) % PLAYER], i[(issuer + 2) % PLAYER],
                trumpmask);
    if (index == w)
      winner = issuer;
  ...
  if (trumpmask & (ONE << index)) {
    ...
    int suit = Suit[index];
    //    cout << "suit/1 " << suit << ": " ; print_vector(Mask[suit] &
    //    ~trumpmask,trumpmask); cout << endl;
  ...
  //    cout << " before reducing/1b "; ::print_extvector(playable,trumpmask);
  //    cout << " " << endl;
  card reduced = playable;
  while (reduced) {
    int index = largest(reduced);
  ...
  //    cout << " after reducing/1b "; ::print_extvector(playable,trumpmask);
  //    cout << " " << endl;
#ifdef TAROT
...
int GamePlay::small_trump(card hand) {
  ...
  if (count(hand) >= 2) {
    ...
    if (VALUE(s) > 10) return small_trump(hand & ~(ONE << s));
  }
#ifdef BELOTE
  if (hand & ~(JACKS | NINES))
  ...
  if (c >= countopponents)
    ...
    for (int j = 0; j < TRUMPCARDTYPES; j++) {
      if ((c > 0) && (hand & abovetrump[j] & trumpmask)) {
        vo |= hand & abovetrump[j] & trumpmask;
        c--;
      }
      //	else {
...
void GamePlay::print_vector(card hand, card trumpmask) {
  ...
  while (hand) {
    ...
    int index = CTZ(hand);
    if ((ONE << index) & trumpmask) std::cout << card_name[index] << " ";
    hand &= ~(ONE << index);
  }
...
int GamePlay::strength(card hand) {
  ...
  score += count(hand & TRUMPS & ~HONOR) + count(hand & TRUMPS & ~HONOR) +
           count(hand & TRUMPS & ~HONOR);

  score += count(hand & ((ONE << t_16) | (ONE << t_17) | (ONE << t_18) |
                         (ONE << t_19) | (ONE << t_20)));
  score += count(hand & ((ONE << t_16) | (ONE << t_17) | (ONE << t_18) |
                         (ONE << t_19) | (ONE << t_20)));
  ...
  for (int i = 0; i < SUITCOUNT; i++) {
    ...
    if (count(hand & Mask[i]) >= 2) score += 1 * count(hand & JACKS & Mask[i]);
