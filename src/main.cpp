...
int main() {
  ...
  for (int seed = 0; seed < 100; seed++) {
    ...
    list<card> list_hands = deal(deck, distrib, seed);
    card hands[N_PLAYERS];
    int i = 0;
    for (auto h : list_hands) {
      hands[i] = h;
      i++;
    }

    // rotation of hands for one dealing
    for (int rot = 0; rot < 1; rot++) {
      Game game(rot);
      for (int p = 0; p < N_PLAYERS; p++) {
        int idx_h = (p - rot + N_PLAYERS) % N_PLAYERS;
        ai[p]->StartGame(idx_h, hands[idx_h]);
        player[p].hand = hands[idx_h];
        for (int i = 0; i < N_PLAYERS; i++)
          player[i].have_not[p] = 0;
      }
